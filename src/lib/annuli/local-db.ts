...
export async function localAllPersons(): Promise<Person[]> {
...
export async function localPutPerson(p: Person): Promise<void> {
  const db = await openDB();
  await wrap(db.transaction(DST, "readwrite").objectStore(DST).put(p));
}

export async function localDelPerson(id: string): Promise<void> {
  const db = await openDB();
  await wrap(db.transaction(DST, "readwrite").objectStore(DST).delete(id));
}

export async function localImgPut(id: string, blob: Blob): Promise<void> {
  const db = await openDB();
  await wrap(db.transaction(IST, "readwrite").objectStore(IST).put({ id, blob }));
}

export async function localImgGet(id: string): Promise<Blob | null> {
  const db = await openDB();
...
export async function localImgDel(id: string): Promise<void> {
  if (!id) return;
  const db = await openDB();
